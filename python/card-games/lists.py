...
def approx_average_is_average(hand):
    ...
    """
    if card_average(hand) == (hand[0]+hand[-1])/2 : return True
    elif card_average(hand) == hand[int(len(hand)/2)] : return True
    return False
...
def average_even_is_average_odd(hand):
    ...
    for index, item in enumerate(hand):
        if index % 2 == 0: even_list.append(item) 
        else: odd_list.append(item)
    if card_average(even_list) == card_average(odd_list): return True
    return False
